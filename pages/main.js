...
const Main = ({ user, setUser }) => {
				...
				.then(async (res) => {
					...
					if (res.data && res.data.order && res.data.order.length !== 0 || res.data.status === 'Delivering') {
						let order = await axios({
							url: `${process.env.NEXT_PUBLIC_BASE_URL}orders/${res.data.order}`,
							method: 'get',
						})
	...
	const orderDelivered = async () => {
		...
		let order = await axios({
			url: `${process.env.NEXT_PUBLIC_BASE_URL}orders/${orderData._id}`,
			method: 'put',
			data: body
		})
		.then(async () => {
			...
			let staff = await axios({
				url: `${process.env.NEXT_PUBLIC_BASE_URL}admin/deliver/${user._id}`,
				method: 'PATCH',
				data: {...staffBody}
			})
			.then((res) => {
